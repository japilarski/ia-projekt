...
```{r wykresy_cen, eval=TRUE, echo = FALSE, fig.height=8}
w1 <- ggplot(euro_pln, aes(x = Data, y = Zamkniecie, group = 1)) +
  geom_line() +
  ggtitle("EUR/PLN") +
  ylab("Cena zamknięcia w PLN")

w2 <- ggplot(mci_pln, aes(x = Data, y = Zamkniecie, group = 1)) +
  geom_line() +
  ggtitle("MCI") +
  ylab("Cena zamknięcia w PLN")
  scale_y_continuous(labels = label_percent(scale = 1))
w3 <- ggplot(pkn_pln, aes(x = Data, y = Zamkniecie, group = 1)) +
  geom_line() +
  ggtitle("PKN") +
  ylab("Cena zamknięcia w PLN")
w4 <- ggplot(xau_pln, aes(x = Data, y = Zamkniecie, group = 1)) +
  geom_line() +
  ggtitle("XAU/PLN") +
  ylab("Cena zamknięcia w PLN")
...
1. Pudełko (box), które reprezentuje zakres między 25. a 75. percentylem (Q1 i Q3), obrazując interkwartylowy zakres (IQR). Pudełko pokazuje, w jakim przedziale znajduje się połowa danych, wskazując na stopień koncentracji wartości wokół mediany.
2. Linia wewnątrz pudełka (mediana), która wskazuje środek rozkładu, dzieląc dane na dwie równe części. Położenie mediany względem pudełka informuje o symetrii rozkładu danych.
3. Wąsy (whiskers), które wychodzą z pudełka, pokazują zakres danych, znajdujących się w obrębie 1.5-krotności interkwartylowego zakresu (IQR) powyżej 75. percentyla i poniżej 25. percentyla. Wąsy wskazują na rozpiętość danych, nie obejmując wartości odstających.
4. Wartości odstające (outliers), które są reprezentowane jako punkty znajdujące się poza zasięgiem wąsów. Te obserwacje są wyraźnie różniące się od reszty danych i wskazują na nietypowe lub ekstremalne wartości w zbiorze.
5. Dodatkowo, na podstawie długości wąsów i położenia mediany, można ocenić asymetrię rozkładu. Jeśli wąsy są nierównej długości, rozkład jest przesunięty, co może sugerować obecność skośności. Wykres pudełkowy pozwala również na ocenę rozproszenia danych — szerokie pudełko wskazuje na większe zróżnicowanie, natomiast wąskie pudełko sugeruje mniejsze rozproszenie.

W skrócie, wykres pudełkowy dostarcza wizualnych informacji o rozkładzie zmiennej, umożliwiając identyfikację jej średniej, zakresu, rozproszenia oraz wartości odstających, a także ocenę symetrii lub asymetrii danych.

```{r boxplot, eval=TRUE, echo = FALSE, fig.height=8}
